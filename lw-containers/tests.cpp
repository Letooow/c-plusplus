...
TEST(nonexceptions, copy_constructors)
{
	ASSERT_TRUE(!noexcept(bs_sizet_t(std::declval< const bs_sizet_t & >())));
	ASSERT_TRUE(!noexcept(bs_string_t(std::declval< const bs_string_t & >())));
	ASSERT_TRUE(!noexcept(bs_nc_t(std::declval< const bs_nc_t & >())));
	ASSERT_TRUE(!noexcept(bs_co_t(std::declval< const bs_co_t & >())));
}

TEST(nonexceptions, move_constructors)
{
	ASSERT_TRUE(noexcept(bs_sizet_t(std::declval< bs_sizet_t && >())));
	ASSERT_TRUE(noexcept(bs_string_t(std::declval< bs_string_t && >())));
	ASSERT_TRUE(noexcept(bs_nc_t(std::declval< bs_nc_t && >())));
	ASSERT_TRUE(noexcept(bs_co_t(std::declval< bs_co_t && >())));
}
